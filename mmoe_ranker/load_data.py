...
def _split_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    print('Splitting dataset by date ranges')
...
def load_data() -> HmData:
    if os.path.exists(os.path.join(OUTPUT_DIR, 'data.pkl')):
        ...
        return pickle.load(open(os.path.join(OUTPUT_DIR, 'data.pkl'), 'rb'))

    print('data not found on disk. Loading data...')

    print('Loading articles')
    article_df = pd.read_csv(r"D:\Study\UNIVERSITY\THIRD YEAR\Business Analytics\final assignment\hm-two-step-reco\data\articles.csv")[Features.ARTICLE_FEATURES]
    for categ_variable in Features.ARTICLE_CATEG_FEATURES:
    ...
    transactions_df = pd.read_csv(r"D:\Study\UNIVERSITY\THIRD YEAR\Business Analytics\final assignment\hm-two-step-reco\data\transactions_train.csv", parse_dates=['t_dat'])
    ...
    transactions_df = enrich_transactions(article_df, customer_df, transactions_df)
    transactions_df[Features.LABEL1] = 1.0

    print('Split data into train and test data')
    train_df, test_df = _split_data(transactions_df)
    ...
    pickle.dump(data, open(os.path.join(OUTPUT_DIR, 'data.pkl'), 'wb'))
