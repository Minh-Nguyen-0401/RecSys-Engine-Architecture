...
def save_dataset(ds, x_filename, y_filename):
    ...
    pickle.dump(x, open(os.path.join(OUTPUT_DIR, x_filename), 'wb'))
    pickle.dump(y, open(os.path.join(OUTPUT_DIR, y_filename), 'wb'))
...
def build_lookups(train_df, article_df) -> Dict[str, tf.keras.layers.StringLookup]:
    ...
    for categ_variable in Features.ALL_CATEG_FEATURES:
        if categ_variable == 'article_id':
            ...
            lookups[categ_variable] = tf.keras.layers.StringLookup(vocabulary=unique_values, num_oov_indices=1)
        else:
...
def preprocess(data: HmData, batch_size: int) -> PreprocessedHmData:
    print('Preprocessing')
    lookups = build_lookups(data.train_df, data.article_df)

    normalization_layers = build_normalization_layers(data)

    total_count = len(data.train_df)
    article_ids = list(data.all_articles_counts.keys())
    article_probs_df = pd.DataFrame({'article_id': article_ids, 'article_counts': [data.all_articles_counts[article_id] for article_id in article_ids]})
    article_probs = article_probs_df['article_counts'].to_numpy()
    article_probs = article_probs / np.sum(article_probs)
    ...
    if os.path.exists(os.path.join(OUTPUT_DIR, 'test_x.p')) and os.path.exists(os.path.join(OUTPUT_DIR, 'test_y.p')):
        test_x = pickle.load(open(os.path.join(OUTPUT_DIR, 'test_x.p'), 'rb'))
        test_y = pickle.load(open(os.path.join(OUTPUT_DIR, 'test_y.p'), 'rb'))
